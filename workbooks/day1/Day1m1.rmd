...
-   If you do not specify the argument names, you must be sure to pass the
    arguments in the correct order. Try **sample(1, die)** and see what happens
    -- the normal order is sample(die, 1)
...
-   In this example, we get 1, repeatedly, since sample() is taking a sample
    from a "vector" containing only the number 1, so that's all it can produce
...
#citation("tidyverse")
>>>>>>> faae0c0f49ed1eb9b2d7e25d55f80a7a3d087920
```
